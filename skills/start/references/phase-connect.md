...
`ls design/`로 Stitch 산출물을 확인한다.
- 파일이 있으면: "Stitch 디자인 N개를 확인했어요" 라고 알린다.
- 없으면: "Stitch 프로토타입을 아직 안 만드셨나요?" 물어본다. 만들 거면 phase-plan의 Stitch
  안내로 잠깐 돌아간다. 건너뛸 거면 그대로 진행한다 (구현 단계에서 화면을 글 설명만으로 만든다).
