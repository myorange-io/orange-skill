...
**이건 새 가입이 아니다** — Supabase는 별도로 미리 가입할 필요가 없고(6단계에서 Vercel
Marketplace가 프로젝트와 계정을 이메일로 자동 생성·연결한다), `supabase login`은 그 계정으로
GitHub 등 SSO 로그인 1회일 뿐이다. 사용자가 불안해하면 이렇게 한 줄로 안심시킨다.
...
**Supabase는 미리 가입할 필요가 없다.** 아래 `vercel integration add supabase`가 Vercel
Marketplace를 통해 Supabase 프로젝트를 만들면서, 사용자 이메일로 **Supabase 계정·조직을 자동
생성·연결**한다(결제도 Vercel로 묶인다). 사용자가 "Supabase도 가입해야 하나요?"라고 물으면
"아니요, Vercel이 자동으로 만들어 줘요"라고 안심시킨다.
