...
### PLAN.md와 맞춰보기 — Stitch에서 늘어난 게 없나

사용자는 Stitch에서 화면을 다듬다가 **새 화면·기능을 추가**하는 경우가 많다 (Stitch가 알아서
넣어 주기도 한다). 이걸 그냥 두면 `PLAN.md`(빌드의 기준)와 실제 디자인이 어긋난다. 정리한
`design/`을 `PLAN.md` `## 화면`과 **비교**해, PLAN에 없는 화면·기능이 있는지 확인한다:

- `design/`의 화면(zip이면 HTML 파일 목록·제목, 스크린샷이면 파일명·내용)을 훑어 화면 목록을 뽑는다.
- `PLAN.md` `## 화면`의 목록과 대조해 **PLAN엔 없고 Stitch에만 있는 화면·기능**을 찾는다.
- 차이가 있으면 사용자에게 한 번 묻는다 (없으면 이 단계는 건너뛴다):
  "Stitch 디자인에 PLAN.md엔 없던 **[화면/기능 이름]**이 보여요. 어떻게 할까요?"
  → **이번 MVP에 포함** / **다음으로 미루기** / **무시(디자인에서 뺌)**.
- 사용자 선택에 따라 `PLAN.md`를 갱신한다:
  - 포함 → `## 화면`에 해당 화면을 추가하고 `## 진행 상황` 체크리스트에 `- [ ] 구현: [화면]`을 넣는다.
    (데이터가 필요하면 `## 데이터`도 보완.)
  - 미루기 → `## MVP 범위`의 '제외 — 다음에'에 적는다.
  - 무시 → `PLAN.md`는 그대로 두고, 구현 단계에서 그 화면은 만들지 않는다.
- **스코프는 사용자가 정한다.** Claude가 임의로 화면을 늘리거나 빼지 않는다 — 차이를 보여주고
  고르게 한다. 차이가 없으면 "PLAN.md와 Stitch 디자인이 일치해요"라고 한 줄로 알리고 넘어간다.
