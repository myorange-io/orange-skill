...
**예외 — Stitch 프로토타이핑은 자동화하지 않는다.** Stitch는 사용자가 **직접 써보며 배우는**
단계다 — 프롬프트 붙여넣기·생성·디자인 선택·zip 다운로드를 모두 사용자가 직접 한다. Claude가
크롬으로 대신하지 않는다(사용자가 "대신 해줘"라고 해도 권하지 않는다). 사용량도 아끼고,
무엇보다 Stitch를 손에 익히는 게 이 단계의 목적이다.
...
- **토큰 비용 인지.** 크롬 제어는 Claude 사용량을 더 쓴다. 그래도 administrative(설정·인증·
  대시보드) 단계에선 사용자 편의를 위해 기본으로 시도한다. Stitch는 직접 써보며 배우는
  단계라 자동화하지 않는다(예외).
